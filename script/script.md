...
+ 脚本名称[115_cli.sh]：调用 115 非 OpenAPI 的私有 Web/离线下载和文件接口，支持云下载任务添加/列表/删除/清空、登录检查、版本查询、新建目录、列目录、获取信息、搜索、删除、移动、复制、重命名、上传、下载；`rm`/`mv`/`cp`/`rename` 支持批量路径参数并在 `--help` 中标注为 batch；默认输出人类友好文本，添加 `--json` 时输出标准化关键字段 JSON（AI调用建议默认加上），添加 `--raw-response` 时输出 115 原始响应；添加云下载任务使用网页端离线下载表单接口，上传命令需要 `python3` 或 `python` 实现 ec115/OSS 流程并支持串行 multipart。必填参数：子命令 `add`、`list`、`delete`、`clear`、`login-check`、`version`、`mkdir`、`ls`、`info`、`search`、`rm`、`mv`、`cp`、`rename`、`upload` 或 `download`，除 `version` 外需要 `--cookie` 或 `PAN115_COOKIE` 环境变量，目录参数使用绝对路径。可选参数：`--dir`、`--path`、`--target-dir`、`--url`、`--keyword`、`--name`、`--output`、`--page`、`--offset`、`--limit`、`--hash`、`--delete-files`、`--flag`、`--multipart-threshold`、`--part-size`、`--json`、`--raw-response`、`--verbose`。示例：`PAN115_COOKIE="UID=...;CID=...;SEID=...;KID=..." script/115_cli.sh add --dir "/下载" --url "https://example.com/file.zip"`；批量重命名：`script/115_cli.sh rename --path "/a.txt" --name "a2.txt" --path "/b.txt" --name "b2.txt" --json`
+ 脚本名称[openlist_cli.sh]：调用 OpenList v4 API 管理离线下载任务和云端文件，支持工具查询、任务添加/列表/删除/清理、目录与文件查询、索引搜索、批量删除/移动/复制/重命名、上传和安全下载；离线下载命令统一使用 `offline-` 前缀；`search` 仅查询 OpenList 已配置的搜索索引，不会实时递归扫描存储，新文件可能需构建或更新索引后才能查到；默认输出人类友好文本，`--json` 输出简化 JSON，`--raw-response` 输出单个原始响应或多请求响应数组；Git Bash 本地路径需使用 `/c/...` 形式，云路径始终使用绝对正斜杠路径。必填参数：子命令 `offline-tools`、`offline-add`、`offline-list`、`offline-delete`、`offline-clear`、`mkdir`、`ls`、`info`、`search`、`rm`、`mv`、`cp`、`rename`、`upload` 或 `download`，所有命令需要 `--base-url` 或 `OPENLIST_BASE_URL`，除 `offline-tools` 外还需要 `--token` 或 `OPENLIST_TOKEN`。可选参数：`--base-url`、`--token`、`--dir`、`--path`、`--target-dir`、`--url`、`--tool`、`--delete-policy`、`--id`、`--keyword`、`--name`、`--file`、`--output`、`--phase`、`--status`、`--page`、`--limit`、`--scope`、`--all`、`--refresh`、`--succeeded-only`、`--overwrite`、`--skip-existing`、`--merge`、`--cancel`、`--as-task`、`--no-overwrite`、`--json`、`--raw-response`、`--verbose`、`--`。示例：`OPENLIST_BASE_URL="https://openlist.example.com" OPENLIST_TOKEN="token" script/openlist_cli.sh ls --dir "/" --json`
