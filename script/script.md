...
+ 脚本名称[openlist_cli.sh]：调用 OpenList v4 API 管理离线下载任务和云端文件，支持工具查询、任务添加/列表/删除/清理、目录与文件查询、索引搜索、批量删除/移动/复制/重命名、上传和安全下载；`search` 仅查询 OpenList 已配置的搜索索引，不会实时递归扫描存储，新文件可能需构建或更新索引后才能查到；默认输出人类友好文本，`--json` 输出简化 JSON，`--raw-response` 输出单个原始响应或多请求响应数组；Git Bash 本地路径需使用 `/c/...` 形式，云路径始终使用绝对正斜杠路径。必填参数：子命令 `tools`、`add`、`list`、`delete`、`clear`、`mkdir`、`ls`、`info`、`search`、`rm`、`mv`、`cp`、`rename`、`upload` 或 `download`，所有命令需要 `--base-url` 或 `OPENLIST_BASE_URL`，除 `tools` 外还需要 `--token` 或 `OPENLIST_TOKEN`。可选参数：`--base-url`、`--token`、`--dir`、`--path`、`--target-dir`、`--url`、`--tool`、`--delete-policy`、`--id`、`--keyword`、`--name`、`--file`、`--output`、`--phase`、`--status`、`--page`、`--limit`、`--scope`、`--all`、`--refresh`、`--succeeded-only`、`--overwrite`、`--skip-existing`、`--merge`、`--cancel`、`--as-task`、`--no-overwrite`、`--json`、`--raw-response`、`--verbose`、`--`。示例：`OPENLIST_BASE_URL="https://openlist.example.com" OPENLIST_TOKEN="token" script/openlist_cli.sh ls --dir "/" --json`
