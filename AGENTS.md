...
+ 图片上传：如果用户需要上传图片到临时的服务器，可以通过调用 `${AiHub Path}/script/upload_images_to_image_server.sh`脚本来上传对应的图片到公共的图片服务器上。最后输出给用户的时候，最好能直接引用这个图片（Markdown，方便用户能直接查看这个图片），同时引用图片下面还需要给出图片的实际链接。
