...
+ 图片相关任务：如果用户命令中包含图片相关的指令，则优先尝试调用 `${AiHub Path}/script/request_thirdparty_ai_platform.sh`，请求三方AI平台模型来完成对应的任务，避免因为读入图片污染上下文，可以优先通过models子命令来确定三方AI平台是否支持能完成对应任务的模型。
  + 图片生成：
    + 模型：gpt-image-2
  + 图片分析：
